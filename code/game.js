...
//Coin now represents winning object to reach at the end of the level/game
function Level(plan) {
...
function Fast(pos) {
  ...
  this.size = new Vector(0.7, 0.5);
  this.wobble = Math.random() * Math.PI * 15;
  this.speed = new Vector(5, 5);
}
Fast.prototype.type = 'faster';
...
if (this.status != null)
  ...
  var wobbleSpeed = 5, wobbleDist = 0.13;
...
var wobbSpeed = 3, wobbDist = 0.4;
    ...
    var wobSpeed = 7, wobDist = 0.3;
        Slow.prototype.act = function(step) {
          this.wobble += step * wobSpeed;
          var wobPos = Math.cos(this.wobble) * wobDist;
          var wobPosY = Math.cos(this.wobble) * wobDist;
          this.pos = this.basePos.plus(new Vector(wobPos, wobPosY));
        };
